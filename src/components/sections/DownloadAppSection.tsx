  ...
  return (
    <section className="py-16 lg:py-24 bg-[#f8fafc] px-4 sm:px-6 lg:px-8 border-t border-gray-100 overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16">
        ...
        {/* Left: Text & Features & Side-by-side Store Badges */}
        <div className="flex flex-col gap-8 w-full lg:w-1/2 text-center lg:text-left">
          ...
          <div className="flex flex-row items-center justify-center lg:justify-start gap-3 sm:gap-4 pt-2 w-full max-w-md mx-auto lg:mx-0">
            <button className="flex-1 sm:flex-initial flex items-center justify-center gap-2.5 px-4 sm:px-6 py-3 rounded-2xl bg-[#071b65] text-white hover:bg-[#0c288d] border border-white/20 transition-all shadow-md">
              <img src={appleLogo} alt="App Store" className="w-6 h-6 sm:w-7 sm:h-7 invert brightness-200" />
              <div className="flex flex-col text-left">
                <span className="text-[9px] sm:text-[10px] text-white/70 uppercase tracking-wider font-sans leading-none">Download on</span>
                <span className="text-[11px] sm:text-xs font-bold font-trajan uppercase tracking-wide whitespace-nowrap">App Store</span>
              </div>
            </button>

            <button className="flex-1 sm:flex-initial flex items-center justify-center gap-2.5 px-4 sm:px-6 py-3 rounded-2xl bg-[#071b65] text-white hover:bg-[#0c288d] border border-white/20 transition-all shadow-md">
              <img src={googlePlay} alt="Google Play" className="w-6 h-6 sm:w-7 sm:h-7" />
              <div className="flex flex-col text-left">
                <span className="text-[9px] sm:text-[10px] text-white/70 uppercase tracking-wider font-sans leading-none">Get it on</span>
                <span className="text-[11px] sm:text-xs font-bold font-trajan uppercase tracking-wide whitespace-nowrap">Google Play</span>
              </div>
            </button>
          </div>
        </div>

        {/* Right: Enlarged Phone Mockup Visual Anchor */}
        <div className="w-full lg:w-1/2 flex justify-center max-w-md lg:max-w-lg">
          <img
            src={mobilePhone}
            alt="Shiloh Word Chapel Official App Preview"
            className="w-full max-h-[580px] lg:max-h-[700px] object-contain drop-shadow-2xl hover:scale-105 transition-transform duration-500"
          />
        </div>
